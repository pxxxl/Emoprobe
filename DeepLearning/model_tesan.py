...
class SAN(nn.Module):
    def __init__(self, in_dim, h_dim):
        ...
        # initial
        nn.init.xavier_normal_(self.Wx.weight, 1)
        nn.init.xavier_normal_(self.Wa.weight, 1)
        nn.init.xavier_normal_(self.Ua.weight, 1)
        nn.init.constant_(self.Wx.bias, 0.0)
        nn.init.constant_(self.Wa.bias, 0.0)
...
class TESA(nn.Module):
    def __init__(self, in_dim, h_dim):
        ...
        # pos embedding
        self.pos_emb = nn.Embedding(700, in_dim)
        # context attention
        ...
        self.drop = nn.Dropout(0.2)
        # initial
        nn.init.xavier_normal_(self.pos_emb.weight, 1)
        nn.init.xavier_normal_(self.Wc.weight, 1)
        nn.init.xavier_normal_(self.V.weight, 1)
        nn.init.constant_(self.Wc.bias, 0.0)
        nn.init.constant_(self.V.bias, 0.0)

    def forward(self, word_vecs, dt_vec, mask):
        # position embedding
        pos_idx = torch.arange(word_vecs.shape[1]).long().to(device)
...
class NTM(nn.Module):
    def __init__(self, net_arch):
        ...
        nn.init.xavier_normal_(self.topic_emb.weight, 1)
        nn.init.xavier_normal_(self.word_emb.weight, 1)
        nn.init.constant_(self.en1_fc.bias, 0.0)
        nn.init.constant_(self.en2_fc.bias, 0.0)
